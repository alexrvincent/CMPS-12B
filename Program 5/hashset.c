...
hashset_ref new_hashset (void) {
   ...
   new->load = 0;
   new->array = malloc (new->size * sizeof (char*));
   assert (new->array != NULL);
   for (size_t index = 0; index < new->size; ++index) {
      new->array[index] = NULL;
   }
   DEBUGF ('h', "%p -> struct hashset {size = %d, array=%p}\n",
                new, new->size, new->array);
   return new;
}

void free_hashset (hashset_ref hashset) {
   DEBUGF ('h', "free (%p), free (%p)\n", hashset->array, hashset);
// Iterate over the hashset, freeing each item in the array
   for(unsigned int i=0; i<hashset -> size; i++)
      if(hashset -> array[i] != NULL)
         free(hashset -> array[i]);
   memset (hashset->array, 0, hashset->size * sizeof (char*));
   free (hashset->array);
...
void doublearray(hashset_ref hashset){
   bool loop = false;
   size_t nlength = 1+ 2* hashset -> size;
   ...
   DEBUGF('h', "Load is: %d\n", hashset -> load);
   fflush(NULL);
   for(hashcode_t count = 0; count < hashset -> size; count++){
   ...
   hashset -> size = nlength;
   free(temp);
}


void put_hashset (hashset_ref hashset, char *item) {
   hashcode_t hash = strhash(item) %hashset -> size;
   bool loop = false;
   if(hashset-> load *(unsigned int)4 > hashset -> size)
      doublearray(hashset);
   while(true){
      if(hashset -> array[hash]== NULL) break;
      if(strcmp(hashset -> array[hash], item)==0) return;
      if(!loop && (hash == hashset -> size-1)){
...
bool has_hashset (hashset_ref hashset, char *item) {
   ...
   hashcode_t hash = strhash(item) % hashset ->size;
   for(unsigned int x=0; x<hashset->size-hash;x++){
...
strhash(hashset -> array[i])% hashset -> size,
 hashset -> array[i]);
}
...
void debug_hash(hashset_ref hashset){
   ...
   printf("%7zd size of hash array\n" , hashset -> size);
   while((unsigned int)pos < hashset -> size){
